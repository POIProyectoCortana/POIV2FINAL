...
public:
	...
	unsigned int planoTextura, planoTextura2, planoTextura3;	

	Terreno(HWND hWnd, WCHAR alturas[], WCHAR textura[], float ancho, float prof, 
		WCHAR textura2[], WCHAR textura3[])
	{
		anchof = ancho;
		proff = prof;
		...
		terreno = Plano3t( Ancho(), Alto(), ancho, prof, Dir_Imagen(), 30, 0.0, 100.0 ,200.0);	
		deltax = anchof / Ancho();
		deltaz = proff / Alto();
		verx=Ancho();
		verz=Alto();
		...
		Carga(textura3);
		glGenTextures(1, &planoTextura3);
		glBindTexture(GL_TEXTURE_2D, planoTextura3);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_REPEAT);
			glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_REPEAT);
			glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST_MIPMAP_LINEAR);
			glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST_MIPMAP_LINEAR);
